# How to Run Ask My Notes

There are two ways to run lesson 1:
...
I do not know. No note matches this question.
...
```bash
python app.py "How long do refunds take?"                  # local model
python app.py "What are the office hours?" --show-search   # show the search results too
python app.py "Which plan do I need for 5 projects?" --hard  # frontier model
python app.py                                              # chat mode; type 'exit' to stop
```
...
The notebook [`ask_my_notes_gcp.ipynb`](ask_my_notes_gcp.ipynb) does all the steps for you:
...
5. In the file browser, open `learn-ai-with-me/lesson1/ask_my_notes_gcp.ipynb`.
...
2. Click **Import** and choose **URL**. Use:
   ...
   ```
   https://github.com/techbyavanti/learn-ai-with-me/blob/main/lesson1/ask_my_notes_gcp.ipynb
   ```
...
| 5–6. Load and search | Embeds the notes and runs the retrieval eval |
| 7. Local model | Asks questions with the local model |
| 8. Frontier model | Asks a question with Claude (only with an API key) |
| 9. Your notes | Replaces the sample notes with your own texts |
...
| The local model says "I do not know" for a question that the notes answer | Small models can be too careful. Run with `--show-search` to check that the correct note is found, then try `--hard` or a larger model (for example `LOCAL_MODEL=qwen2.5:7b-instruct`). |
